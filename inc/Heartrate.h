...
private:
    ...
    void on_init_complete(BLE::InitializationCompleteCallbackContext *params)
    {
        if (params->error != BLE_ERROR_NONE) {
            LOG_DEBUG("%s", "Ble initialization failed.");
            return;
        }
    ...
    void start_advertising()
    {
        ...
        if (error) {
            LOG_DEBUG("%s", "_ble.gap().setAdvertisingParameters() failed\r\n");
            return;
        }
        ...
        if (error) {
            LOG_DEBUG("%s", "_ble.gap().setAdvertisingPayload() failed\r\n");
            return;
        }
        ...
        if (error) {
            LOG_DEBUG("%s", "_ble.gap().startAdvertising() failed\r\n");
            return;
        }

        LOG_DEBUG("%s", "Heart rate sensor advertising, please connect\r\n");
    }
...
private:
    ...
    virtual void onConnectionComplete(const ble::ConnectionCompleteEvent &event)
    {
        if (event.getStatus() == ble_error_t::BLE_ERROR_NONE) {
            LOG_DEBUG("%s", "Client connected, you may now subscribe to updates\r\n");
        }
    }
    ...
    {
        LOG_DEBUG("%s", "Client disconnected, restarting advertising\r\n");
